 ...
 */
async function resolveAdminModuleId(client, value) {
    const modules = await client.get("/api/admin/modules");
    const lower = value.trim().toLowerCase();
    const match = modules.find((m) => m.id === value || m.slug.toLowerCase() === lower || m.label.toLowerCase() === lower);
    if (!match) {
        const valid = modules
            .map((m) => m.slug)
            .sort()
...
export function registerTools(server, client) {
    ...
    // pulse_search_issues
    // ===========================================================
    server.tool("pulse_search_issues", "Search/filter Pulse issues by status, priority, category, module, assignee, " +
        ...
        status: z
            .string()
            .optional()
            .describe("Filter by status (BACKLOG|OPEN|IN_PROGRESS|STAGING|IN_REVIEW|RESOLVED|CLOSED). Passed through as-is — the server is authoritative."),
        ...
        includeReleased: z
            .boolean()
            .optional()
            .describe("Include issues in RELEASED milestones. Pulse hides these by default in every other filter combination. Default false."),
        limit: z
            .number()
            .int()
            .positive()
        ...
        const issues = await client.get("/api/issues", query);
        return textResult({ count: issues.length, issues: issues.map(compactIssue) });
    });
    // ===========================================================
    // pulse_get_issue
    // ===========================================================
    server.tool("pulse_get_issue", "Fetch full details for a single Pulse issue by key (e.g. PULSE-0001) or id. " +
        ...
        const detail = await client.get(`/api/issues/${encodeURIComponent(id)}`);
        return textResult({
            ...detail,
    ...
    // pulse_list_lookups
    // ===========================================================
    server.tool("pulse_list_lookups", "List reference data used to resolve filter/create values: modules, users, " +
        ...
        const data = await client.get(path[kind]);
        return textResult(data);
    });
    // ===========================================================
    // pulse_code_refs_report
    // ===========================================================
    server.tool("pulse_code_refs_report", "Flat report of code references (PRs/MRs/commits) linked to issues, joined " +
        ...
        const items = await client.get("/api/code-refs", {
            from,
            to,
            provider,
            repo,
    ...
    // pulse_add_code_ref
    // ===========================================================
    server.tool("pulse_add_code_ref", "Attach a PR/MR/commit URL to a Pulse issue as a code reference. Accepts an " +
        ...
        try {
            ...
            if (title !== undefined)
                body.title = title;
            const created = await client.post(`/api/issues/${encodeURIComponent(id)}/code-refs`, body);
    ...
    // pulse_update_issue
    // ===========================================================
    server.tool("pulse_update_issue", "Update fields on an existing Pulse issue. Only the fields you pass are " +
        ...
        module: z
            .string()
            .optional()
            .describe('New module slug or label to reassign the issue to — see pulse_list_lookups(kind: "modules"). Cannot be cleared, only reassigned; an empty string is rejected.'),
        assignee: z
            .string()
            .optional()
    ...
    }, async ({ issue, title, description, category, status, priority, module, assignee, milestone, sprint, dueDate, devStartDate, devDueDate, eusStartDate, eusDueDate, }) => {
        try {
            const id = await resolveIssueId(client, issue);
            const body = {};
            if (title !== undefined)
            ...
            if (module !== undefined) {
                if (module === "") {
                    throw new Error('Module cannot be cleared — provide a valid slug (see pulse_list_lookups(kind: "modules")).');
                }
            ...
            if (milestone !== undefined)
                body.milestoneId = milestone === "" ? null : milestone;
            if (sprint !== undefined)
                body.sprintId = sprint === "" ? null : sprint;
            if (dueDate !== undefined)
                body.dueDate = clearableDate(dueDate);
            if (devStartDate !== undefined)
                body.devStartDate = clearableDate(devStartDate);
            if (devDueDate !== undefined)
                body.devDueDate = clearableDate(devDueDate);
            if (eusStartDate !== undefined)
                body.eusStartDate = clearableDate(eusStartDate);
            if (eusDueDate !== undefined)
                body.eusDueDate = clearableDate(eusDueDate);
            if (Object.keys(body).length === 0) {
                throw new Error("No fields provided to update — pass at least one field.");
            }
            const updated = await client.put(`/api/issues/${encodeURIComponent(id)}`, body);
    ...
    // pulse_create_issue
    // ===========================================================
    server.tool("pulse_create_issue", "Create a new Pulse issue. Title, description, and category are " +
        ...
        module: z
            .string()
            .optional()
            .describe('Module slug or label to file the issue under — see pulse_list_lookups(kind: "modules"). Defaults to the server-configured default module.'),
        ...
        try {
            ...
            if (module !== undefined)
                body.module = await resolveModuleSlug(client, module);
            if (assignee !== undefined)
                body.assigneeId = await resolveUserId(client, assignee);
            if (milestone !== undefined)
                body.milestoneId = milestone;
            if (sprint !== undefined)
                body.sprintId = sprint;
            if (dueDate !== undefined)
                body.dueDate = parseDueDate(dueDate) || undefined;
            if (labels !== undefined && labels.length > 0) {
                body.labelIds = await Promise.all(labels.map((l) => resolveLabelId(client, l)));
            }
            const created = await client.post("/api/issues", body);
            return textResult(created);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_add_comment
    // ===========================================================
    server.tool("pulse_add_comment", "Add a comment to a Pulse issue. Plain text is auto-wrapped in a <p> " +
        ...
        try {
            ...
            const created = await client.post("/api/comments", {
                ticketId: id,
                content: html,
            });
            return textResult(created);
    ...
    // ===========================================================
    server.tool("pulse_delete_comment", "Delete a Pulse comment permanently. This is a hard delete with no undo, " +
        ...
        try {
            const result = await client.del(`/api/comments/${encodeURIComponent(comment)}`);
    ...
    // pulse_delete_issue
    // ===========================================================
    server.tool("pulse_delete_issue", "Move a Pulse issue to Trash (soft delete). It is reversible: an admin " +
        ...
        try {
            const id = await resolveIssueId(client, issue);
            const result = await client.del(`/api/issues/${encodeURIComponent(id)}`);
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // ===========================================================
    server.tool("pulse_set_issue_labels", "Replace the full label set on a Pulse issue. This is a full REPLACE, " +
        ...
        labels: z
            .array(z.string())
            .describe('Complete desired list of label ids or exact label names — see pulse_list_lookups(kind: "labels"). Replaces the existing set entirely; pass [] to clear all labels.'),
    }, async ({ issue, labels }) => {
        try {
            const id = await resolveIssueId(client, issue);
            const labelIds = await Promise.all(labels.map((l) => resolveLabelId(client, l)));
            const result = await client.put(`/api/issues/${encodeURIComponent(id)}/labels`, { labelIds });
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_link_issues
    // ===========================================================
    server.tool("pulse_link_issues", "Create a link between two Pulse issues (e.g. mark one as blocking " +
        ...
        try {
            ...
            const link = await client.post(`/api/issues/${encodeURIComponent(id)}/links`, {
                targetId,
    ...
    // pulse_unlink_issue
    // ===========================================================
    server.tool("pulse_unlink_issue", "Remove a link from a Pulse issue by link id (find the link id via " +
        ...
        try {
            const id = await resolveIssueId(client, issue);
            const result = await client.del(`/api/issues/${encodeURIComponent(id)}/links/${encodeURIComponent(linkId)}`);
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_set_assignee
    // ===========================================================
    server.tool("pulse_set_assignee", "Set or clear the assignee on a Pulse issue. This is the dedicated " +
        ...
        try {
            ...
            const result = await client.patch(`/api/issues/${encodeURIComponent(id)}/assignee`, { assigneeId });
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_watch_issue
    // ===========================================================
    server.tool("pulse_watch_issue", "Start watching a Pulse issue (subscribes the authenticated user/token " +
        ...
        try {
            const id = await resolveIssueId(client, issue);
            const result = await client.post(`/api/issues/${encodeURIComponent(id)}/watch`);
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_unwatch_issue
    // ===========================================================
    server.tool("pulse_unwatch_issue", "Stop watching a Pulse issue (unsubscribes the authenticated " +
        ...
        try {
            const id = await resolveIssueId(client, issue);
            const result = await client.del(`/api/issues/${encodeURIComponent(id)}/watch`);
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_move_issue
    // ===========================================================
    server.tool("pulse_move_issue", "Move a Pulse issue to a different module, re-homing it under a new key " +
        ...
        try {
            ...
            const body = { module: targetModule };
            if (reporter !== undefined)
                body.reporterId = await resolveUserId(client, reporter);
            const moved = await client.post(`/api/issues/${encodeURIComponent(id)}/move`, body);
    ...
    // ===========================================================
    server.tool("pulse_create_milestone", "Create a new Pulse milestone. name, targetDate, and module are " +
        ...
        status: z
            .string()
            .optional()
            .describe("Status: PLANNED|ACTIVE|COMPLETED|OPEN|IN_PROGRESS|FOR_TESTING|DONE|FOR_STAGING|FOR_DEPLOYMENT|RELEASED. Defaults to PLANNED."),
        eusLead: z
            .string()
            .optional()
        ...
        labels: z
            .array(z.string())
            .optional()
            .describe('Label ids or exact label names to attach at creation time — see pulse_list_lookups(kind: "labels")'),
        ...
        try {
            ...
            if (eusLead !== undefined)
                body.eusLeadId = await resolveUserId(client, eusLead);
            if (labels !== undefined && labels.length > 0) {
                body.labelIds = await Promise.all(labels.map((l) => resolveLabelId(client, l)));
            }
            const created = await client.post("/api/milestones", body);
            return textResult(created);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // ===========================================================
    server.tool("pulse_update_milestone", "Update fields on an existing Pulse milestone. Only the fields you " +
        ...
        targetDate: z
            .string()
            .optional()
            .describe("New target date (YYYY-MM-DD or ISO datetime). Cannot be cleared — omit to leave unchanged."),
        module: z
            .string()
            .optional()
            .describe('New module slug or label to reassign the milestone to — see pulse_list_lookups(kind: "modules"). Cannot be cleared, only reassigned; an empty string is rejected.'),
        status: z
            .string()
            .optional()
            .describe("New status: PLANNED|ACTIVE|COMPLETED|OPEN|IN_PROGRESS|FOR_TESTING|DONE|FOR_STAGING|FOR_DEPLOYMENT|RELEASED"),
        eusLead: z
            .string()
            .optional()
        ...
        try {
            ...
            if (targetDate !== undefined) {
                if (targetDate === "") {
                    throw new Error("targetDate cannot be cleared — provide a valid date, or omit this field to leave it unchanged.");
                }
            ...
            if (releaseNotes !== undefined)
                body.releaseNotes = releaseNotes === "" ? null : releaseNotes;
            if (Object.keys(body).length === 0) {
                throw new Error("No fields provided to update — pass at least one field.");
            }
            const updated = await client.put(`/api/milestones/${encodeURIComponent(milestone)}`, body);
            return textResult(updated);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // ===========================================================
    server.tool("pulse_delete_milestone", "Delete a Pulse milestone permanently, including its attachments. " +
        ...
        try {
            const result = await client.del(`/api/milestones/${encodeURIComponent(milestone)}`);
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_create_sprint
    // ===========================================================
    server.tool("pulse_create_sprint", "Create a new Pulse sprint. name, startDate, and endDate are " +
        ...
        try {
            ...
            const created = await client.post("/api/sprints", body);
            return textResult(created);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_update_sprint
    // ===========================================================
    server.tool("pulse_update_sprint", "Update fields on an existing Pulse sprint. Only the fields you pass " +
        ...
        module: z
            .string()
            .optional()
            .describe('New module slug or label — see pulse_list_lookups(kind: "modules"). Pass "" to clear (unscope the sprint).'),
        ...
        try {
            ...
            if (startDate !== undefined) {
                if (startDate === "") {
                    throw new Error("startDate cannot be cleared — provide a valid date, or omit this field to leave it unchanged.");
                }
                body.startDate = parseDueDate(startDate);
            }
            if (endDate !== undefined) {
                if (endDate === "") {
                    throw new Error("endDate cannot be cleared — provide a valid date, or omit this field to leave it unchanged.");
                }
            ...
            if (summaryNotes !== undefined)
                body.summaryNotes = summaryNotes === "" ? null : summaryNotes;
            if (Object.keys(body).length === 0) {
                throw new Error("No fields provided to update — pass at least one field.");
            }
            const updated = await client.put(`/api/sprints/${encodeURIComponent(sprint)}`, body);
            return textResult(updated);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_delete_sprint
    // ===========================================================
    server.tool("pulse_delete_sprint", "Delete a Pulse sprint permanently, including its attachments. Issues " +
        ...
        try {
            const result = await client.del(`/api/sprints/${encodeURIComponent(sprint)}`);
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // ===========================================================
    server.tool("pulse_add_sprint_issues", "Add one or more Pulse issues to a sprint. The underlying API accepts " +
    ...
    // ===========================================================
    server.tool("pulse_remove_sprint_issue", "Remove a single Pulse issue from a sprint (clears its sprintId; the " +
        ...
        try {
            const id = await resolveIssueId(client, issue);
            const result = await client.request("DELETE", `/api/sprints/${encodeURIComponent(sprint)}/tickets`, { jsonBody: { ticketId: id } });
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_create_module
    // ===========================================================
    server.tool("pulse_create_module", "Create a new Pulse module (admin-level configuration, not a regular " +
        ...
        prefix: z
            .string()
            .describe("Unique issue-key prefix — 2-10 uppercase letters, or 1-8 letters plus 1-2 trailing digits, no hyphens (e.g. BILL)"),
        ...
        try {
            const body = { slug, label, inkHex, tintHex, prefix };
            ...
            if (status !== undefined)
                body.status = status.toUpperCase();
            const created = await client.post("/api/admin/modules", body);
            return textResult(created);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_update_module
    // ===========================================================
    server.tool("pulse_update_module", "Update fields on an existing Pulse module. slug is NOT patchable " +
        ...
        prefix: z
            .string()
            .optional()
            .describe("New unique issue-key prefix — 2-10 uppercase letters, or 1-8 letters plus 1-2 trailing digits, no hyphens"),
        ...
        try {
            ...
            const updated = await client.patch(`/api/admin/modules/${encodeURIComponent(id)}`, body);
            return textResult(updated);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_delete_module
    // ===========================================================
    server.tool("pulse_delete_module", "Delete a Pulse module. Blocked with a 409 if any issue, milestone, or " +
        ...
        try {
            const id = await resolveAdminModuleId(client, module);
            const result = await client.del(`/api/admin/modules/${encodeURIComponent(id)}`);
            return textResult(result);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // ===========================================================
    server.tool("pulse_create_feedback", "Submit product feedback (feature request, bug report, or nice-to-have " +
        ...
        try {
            const created = await client.post("/api/feedback", {
                title,
                description,
                type: type.toUpperCase(),
            });
    ...
    // pulse_start_standup
    // ===========================================================
    server.tool("pulse_start_standup", "Start a new Pulse daily-standup session for a given date, paging " +
        ...
        try {
            ...
            if (force !== undefined)
                body.force = force;
            if (excludedModuleIds !== undefined)
                body.excludedModuleIds = excludedModuleIds;
            const created = await client.post("/api/standup/sessions", body);
            return textResult(created);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // ===========================================================
    server.tool("pulse_set_standup_notes", "Write (overwrite) the free-text notes on a daily-standup session page " +
        ...
        try {
            const updated = await client.put(`/api/standup/pages/${encodeURIComponent(pageId)}/notes`, { notesHtml });
            return textResult(updated);
        }
        catch (err) {
            if (err instanceof PulseApiError) {
    ...
    // pulse_get_changelog
    // ===========================================================
    server.tool("pulse_get_changelog", "Fetch the Pulse product changelog. By default returns the current " +
