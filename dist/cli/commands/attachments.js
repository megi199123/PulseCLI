...
export function register(program, ctx) {
    ...
    attachmentCmd
        ...
        if (!ctx.json) {
            ...
            if (stat.size > MAX_SIZE_BYTES) {
                info(`Warning: file is ${humanSize(stat.size)}, which exceeds the 10 MB limit. The server may reject it.`);
            }
        }
        // Resolve issue ref → cuid id
        const ticketId = await resolveIssueId(ctx.client, ref);
        const attachment = await ctx.client.uploadFile("/api/attachments", filePath, { ticketId });
        if (ctx.json) {
    ...
    attachmentCmd
        ...
        const { filename } = await ctx.client.downloadFile(`/api/attachments/${attachmentId}?download=1`, tempPath);
    ...
    attachmentCmd
        ...
        if (!opts.yes && !ctx.json) {
            const confirmed = await promptConfirm(`Remove attachment ${attachmentId}?`);
            if (!confirmed) {
                info("Aborted.");
                return;
            }
        }
        const result = await ctx.client.del(`/api/attachments/${attachmentId}`);
        if (ctx.json) {
