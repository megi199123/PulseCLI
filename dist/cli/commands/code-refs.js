 ...
 */
export function formatCodeRefLabel(ref) {
    if (ref.kind === "PR" && ref.number != null)
        return `${ref.repo}#${ref.number}`;
    if (ref.kind === "MR" && ref.number != null)
        return `${ref.repo}!${ref.number}`;
    if (ref.kind === "COMMIT" && ref.sha)
        return `${ref.repo}@${ref.sha.slice(0, 7)}`;
...
export function register(program, ctx) {
    ...
    codeRefCmd
        ...
        const codeRefs = await ctx.client.get(`/api/issues/${encodeURIComponent(issueId)}/code-refs`);
        if (ctx.json) {
            printJson(codeRefs);
        }
        else {
    ...
    codeRefCmd
        ...
        const created = await ctx.client.post(`/api/issues/${encodeURIComponent(issueId)}/code-refs`, body);
        if (ctx.json) {
            printJson(created);
        }
        else {
    ...
    codeRefCmd
        ...
        const issueId = await resolveIssueId(ctx.client, ref);
        await ctx.client.del(`/api/issues/${encodeURIComponent(issueId)}/code-refs/${refId}`);
        if (ctx.json) {
            printJson({ ok: true });
        }
        else {
    ...
    codeRefCmd
        ...
        const items = await ctx.client.get("/api/code-refs", query);
        ...
        if (items.length === 1000) {
            console.error("Warning: 1000 rows returned — this is the server's silent cap; results may be truncated.");
        }
        if (ctx.json) {
            printJson(items);
        }
