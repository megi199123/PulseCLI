...
export function register(program, ctx) {
    ...
    mcpCmd
        .command("setup")
        .description("Interactive setup for the pulse-mcp server: log in, mint an API token, " +
...
async function runSetup(ctx, opts) {
    ...
    if (!user) {
        ...
        if (!interactive && (!email || !password)) {
            throw new Error("Not logged in — run `pulse mcp setup` in an interactive terminal, " +
    ...
    if (grantCodeRefs && interactive) {
        grantCodeRefs = await promptConfirm("Allow the MCP server to attach PR/commit links to issues (CODE_REF_WRITE scope)?", true);
    }
    ...
    ok(`Minted API token "${minted.name}" (${minted.tokenPrefix}…) — ` +
    ...
    saveConfig(updated);
    info("Token stored in the CLI config — pulse-mcp uses it automatically " +
    ...
    const bearerClient = new PulseClient({ baseUrl, cookies: {}, token: minted.token }, { persist: false });
    const modules = await bearerClient.get("/api/modules");
    ok(`Verified: bearer token works against ${baseUrl} ` +
    ...
    if (opts.register) {
        let doRegister = true;
        if (interactive) {
            doRegister = await promptConfirm("Register pulse-mcp with Claude Code now (user scope)?", true);
        }
        if (doRegister)
            registered = registerWithClaude();
    }
    info("Done. Start a NEW Claude Code session (MCP servers spawn at session " +
...
function registerWithClaude() {
    ...
    if (/already exists/i.test(output)) {
        info("A 'pulse' MCP server is already registered with Claude Code — kept as-is.\n" +
            "  If it still uses the old -e PULSE_TOKEN registration, re-add it token-free:\n" +
            `  claude mcp remove pulse -s user && ${REGISTER_COMMAND}`);
        return true;
    }
    info("Could not run the Claude Code CLI (is `claude` on your PATH?). " +
