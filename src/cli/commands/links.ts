...
const VALID_LINK_TYPES: LinkType[] = ["RELATED", "BLOCKS", "BLOCKED_BY"];
...
export function register(program: Command, ctx: CliContext): void {
  ...
  linkCmd
    ...
    .action(async (ref: string) => {
      const links = await ctx.client.get<IssueLink[]>(
        `/api/issues/${encodeURIComponent(ref)}/links`,
      );
  ...
  linkCmd
    ...
    .description(
      "Add a link between two issues (type: RELATED | BLOCKS | BLOCKED_BY)",
    )
    .action(async (sourceRef: string, targetRef: string, typeArg: string) => {
      // Validate type before any network call
      const type = typeArg.toUpperCase() as LinkType;
      if (!VALID_LINK_TYPES.includes(type)) {
        throw new Error(
          `Invalid link type "${typeArg}". Must be one of: ${VALID_LINK_TYPES.join(", ")}`,
        );
      }
      ...
      const link = await ctx.client.post<IssueLink>(
        `/api/issues/${encodeURIComponent(sourceRef)}/links`,
        { targetId, type },
      );
  ...
  linkCmd
    ...
    .action(
      async (ref: string, linkId: string, opts: { yes?: boolean }) => {
        ...
        const result = await ctx.client.del<{ message: string }>(
          `/api/issues/${encodeURIComponent(ref)}/links/${linkId}`,
        );
